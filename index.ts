...
let studentData: StudentData | any = null;
let condition = true;
...
while (condition) {
  let studentInformition = await inquirer.prompt({
    ...
    message: chalk.blueBright.bold(
      "......please select one option and continue opration......."
    ),
    choices: [
      "Add New Student",
      "Check Balance",
  ...
  if (studentInformition.name === "Add New Student") {
    ...
    if (studentName.name !== "") {
      let studentId = await inquirer.prompt({
        ...
        type: "number",
        message: chalk.blueBright.bold(
          "please enter student id and id number is must be (....5 digits)."
        ),
      });
      ...
      if (studentId.id > 1111 && studentId.id < 100000) {
        let bankAccountNumber = await inquirer.prompt({
          ...
          message: chalk.greenBright.bold(
            "please enter bank account number and number is must be (...8 digits)."
          ),
        });

        if (
          bankAccountNumber.account > 1111111 &&
          ...
          let coursesList = await inquirer.prompt({
            ...
            type: "list",
            message: chalk.blueBright.bold(
              "please enter one option and continue opration"
            ),
          ...
          if (coursesList.options === "Free Courses") {
            let freeCoursesList = await inquirer.prompt({
              ...
              type: "list",
              message: chalk.greenBright.bold(
                "please select one option and select course"
              ),
              choices: ["PIAIC", "GIAIC"],
            });
          ...
          console.log(
            chalk.redBright.bold(
              "....!  sorry you wrong entered --- only 8 digits are requierd."
            )
          );
        }
      } else {
        console.log(
          chalk.redBright.bold(
            "....!  sorry you wrong entered --- only 5 digits are requierd."
          )
        );
      }
    } else {
    ...
    let showBalance = await inquirer.prompt({
      ...
      type: "number",
      message: chalk.greenBright.bold(
        "do you want to show balance so enter same account (...8 digits)."
      ),
    });

    if (showBalance.show === studentData?.BankAccount) {
      console.log(
        chalk.greenBright.bold("student balance is :"),
        chalk.yellowBright.bold(10000000)
      );
    } else {
      console.log(
        chalk.redBright.bold(
          "....!  sorry you wrong entered please same number enter --- only 8 digits are requierd."
        )
      );
    }
    ...
    let tuitionFees = await inquirer.prompt({
      ...
      type: "input",
      message: chalk.greenBright.bold(
        "pay tuition fees so enter same student name"
      ),
    });

    if (tuitionFees.fees === studentData?.StudentName) {
      console.log(
        chalk.greenBright.bold("student per month fees is :"),
        chalk.yellowBright.bold(50000)
      );
    } else {
      console.log(
        chalk.redBright.bold("....!  please enter same student name")
      );
    }
    ...
    let enterId = await inquirer.prompt({
      ...
      type: "number",
      message: chalk.greenBright.bold(
        "please enter same student id number (...5 digits)."
      ),
    });
    ...
    if (enterId.id === studentData?.StudentId) {
      ...
      console.log(
        chalk.redBright.bold(
          "....!  sorry you wrong entered please same id enter --- only 5 digits are requierd."
        )
      );
    }
  }
